...
class TransportTab():
    ...
    def __init__(self, tab_parent, komponenten_mit_typ_transport):
        if not komponenten_mit_typ_transport:
            komponenten_mit_typ_transport = ['None']
        self.TYP = 'Transport'
    ...
    def funktion_komponente_umleiten(self, tab_transport, komponenten_mit_typ_transport):
        komponenten_mit_typ_transport = ['None'] + sorted(komponenten_mit_typ_transport)

        var_komponente_typ_transport_von = tk.StringVar()
        var_komponente_typ_transport_von.set('None')
        var_komponente_typ_transport_zu = tk.StringVar()
        var_komponente_typ_transport_zu.set('None')
        ...
        auswahl_komponente_typ_transport_von.grid(row=0, column=1, padx=10, pady=10, sticky='W')

        label_zu = tk.Label(tab_transport, text = 'zu')
        label_zu.grid(row=0, column=2, padx=10, pady=10, sticky='W')
        ...
        auswahl_komponente_typ_transport_zu.grid(row=0, column=3, padx=10, pady=10, sticky='W')
    ...
    def cmd_funktion_umleiten(self, von_komponente, zu_komponente):
        ...
        if von_komponente != 'None' and zu_komponente != 'None':
